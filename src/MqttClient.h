 ...
 * Based on Eclipse Paho MQTT Client
...
#if MQTT_LOG_ENABLED
#define MQTT_LOG_PRINTFLN(fmt, ...)                                            \
    mLogger.printfln_P(MQTT_PSTR("MQTT - " fmt), ##__VA_ARGS__)
#else
...
public:
    ...
    template <class Log> class LoggerImpl : public Logger
    {
    public:
    ...
    public:
         ...
         */
        virtual int
        read(unsigned char *buffer, int len, unsigned long timeoutMs) = 0;
         ...
         * @param timeoutMs - maximum time of bytes processing by network, in
         * milliseconds
         * @return number of written bytes
         */
        virtual int
        write(unsigned char *buffer, int len, unsigned long timeoutMs) = 0;
    ...
    template <class Net> class NetworkImpl : public Network
    {
    public:
        NetworkImpl(Net &network, System &system)
            : mNetwork(network), mSystem(system)
        {
        }
    ...
    template <class Client> class NetworkClientImpl : public Network
    {
    public:
        NetworkClientImpl(Client &client, System &system)
            : mClient(client), mSystem(system)
        {
        }
        ...
        int write(unsigned char *buffer, int len, unsigned long timeoutMs)
        {
            ...
            mClient.setTimeout(timeoutMs);
            // Write
            return connected() ? mClient.write((const uint8_t *)buffer, len)
                               : -1;
        }
    ...
    template <int BUFFER_SIZE = 100> struct ArrayBuffer : public Buffer
    {
        unsigned char *get()
        {
            return buf;
    ...
    struct MessageHandler
    {
        ...
        /** Checks handler association with the topic/callback. True if
         * associated */
        bool isUsed() const
        {
            return topic != NULL;
    ...
    template <int SIZE = 5> class MessageHandlersImpl : public MessageHandlers
    {
    public:
        MessageHandlersImpl()
        {
        ...
        void reset(const char *topic)
        {
            ...
            {
                if (handlers[i].isUsed() &&
                    strcmp(handlers[i].topic, topic) == 0)
                {
                    onDeAllocateTopic(handlers[i].topic, i);
                    handlers[i].reset();
                    break;
     ...
     * @param network - used to send/receive data to/from broker, see
     * NetworkImpl
     * @param sendBuffer - buffer to temporarily store the transmitted message,
     * see ArrayBuffer
     * @param recvBuffer - buffer to temporarily store the received message, see
     * ArrayBuffer
     * @param messageHandlers - storage for subscription callback functions, see
     * MessageHandlersImpl
     * @param netMinTmMs - the minimum amount of time allowed for single network
     * operation, in milliseconds
     */
    MqttClient(const Options &options,
               Logger &logger,
               System &system,
               Network &network,
               Buffer &sendBuffer,
        ...
        : NET_MIN_TM_MS(netMinTmMs), mOptions(options), mLogger(logger),
          mSystem(system), mNetwork(network), mSendBuffer(sendBuffer),
          mRecvBuffer(recvBuffer), mMessageHandlers(messageHandlers),
          mSession(system)
    {
    }
     ...
     */
    Error::type connect(const MQTTPacket_connectData &connectOptions,
                        ConnectResult &result)
    {
        Timer timer(mSystem, mOptions.commandTimeoutMs);
        MQTT_LOG_PRINTFLN("Connect, clean-session: %u, ts: %lu",
        ...
        mSession.reset();
        // Make actual keep-alive timer slightly shorter to avoid expiration on
        // broker side
        mSession.keepAliveTmSec =
            adjustKeepaliveTm(connectOptions.keepAliveInterval);
        int len = 0;
        if ((len = MQTTSerialize_connect(
                 mSendBuffer.get(),
                 mSendBuffer.size(),
                 &(MQTTPacket_connectData &)connectOptions)) <= 0)
        {
        ...
        if (rc == Error::SUCCESS)
        {
            ...
            if (MQTTDeserialize_connack(&ackSessionPresent,
                                        &ackRc,
                                        mRecvBuffer.get(),
                                        mRecvBuffer.size()) != 1)
            {
            ...
            mSession.isConnected =
                (result.code == ConnectResult::Code::SUCCESS);
            if (isConnected())
            {
                ...
                MQTT_LOG_PRINTFLN("Keepalive interval: %u sec",
                                  mSession.keepAliveTmSec);
                // Process session-present flag
                if (!result.sessionPresent)
                {
                    MQTT_LOG_PRINTFLN(
                        "Session is not present => reset subscription");
        ...
        {
            MQTT_LOG_PRINTFLN("Connect ack is not received, rc: %i, ts: %lu",
                              rc,
                              mSystem.millis());
            return rc;
        }
    ...
    Error::type disconnect()
    {
        ...
        int len =
            MQTTSerialize_disconnect(mSendBuffer.get(), mSendBuffer.size());
        if (len <= 0)
        {
            return Error::ENCODING_FAILURE;
        }
    ...
    Error::type publish(const char *topic, Message &message)
    {
        Timer timer(mSystem, mOptions.commandTimeoutMs);
        MQTT_LOG_PRINTFLN(
            "Publish, to: %s, size: %u", topic, message.payloadLen);
        if (!isConnected())
        {
        ...
        int len = MQTTSerialize_publish(mSendBuffer.get(),
                                        mSendBuffer.size(),
                                        0,
                                        message.qos,
                                        message.retained,
        ...
        if (rc == Error::SUCCESS)
            ...
            case QOS1:
            {
                ...
                if (rc == Error::SUCCESS)
                {
                    ...
                    if (MQTTDeserialize_ack(&ackType,
                                            &ackDup,
                                            &ackId,
                                            mRecvBuffer.get(),
                                            mRecvBuffer.size()) != 1)
            ...
            case QOS2:
            {
                ...
                if (rc == Error::SUCCESS)
                {
                    ...
                    {
                        int len = MQTTSerialize_ack(mSendBuffer.get(),
                                                    mSendBuffer.size(),
                                                    PUBREL,
                                                    0,
                                                    ackId);
     ...
     *  @param cbk - the callback function to be invoked when a message is
     * received for this subscription
     *  @return execution status code
     */
    Error::type
    subscribe(const char *topic, enum QoS qos, MessageHandlerCbk cbk)
    {
        Timer timer(mSystem, mOptions.commandTimeoutMs);
        ...
        int len = 0;
        {
            ...
            int mqttQos = qos;
            len = MQTTSerialize_subscribe(mSendBuffer.get(),
                                          mSendBuffer.size(),
                                          0,
                                          mPacketId.getNext(),
                                          1,
        ...
        else
        {
            ...
            else
            {
                ...
                if (rc == Error::SUCCESS)
                {
                    ...
                    unsigned short ackId;
                    if (MQTTDeserialize_suback(&ackId,
                                               1,
                                               &ackCount,
                                               &ackQoS,
                                               mRecvBuffer.get(),
                ...
                {
                    MQTT_LOG_PRINTFLN(
                        "Subscribe ack is not received, rc: %i, ts: %lu",
                        rc,
                        mSystem.millis());
                }
            }
    ...
    Error::type unsubscribe(const char *topic)
    {
        ...
        MQTTString mqttTopic = {(char *)topic, {0, 0}};
        int len = MQTTSerialize_unsubscribe(mSendBuffer.get(),
                                            mSendBuffer.size(),
                                            0,
                                            mPacketId.getNext(),
                                            1,
        ...
        if (rc == Error::SUCCESS)
        {
            ...
            unsigned short ackId;
            if (MQTTDeserialize_unsuback(
                    &ackId, mRecvBuffer.get(), mRecvBuffer.size()) != 1)
            {
                rc = Error::DECODING_FAILURE;
            }
            else
        ...
        {
            MQTT_LOG_PRINTFLN(
                "Unsubscribe ack is not received, rc: %i, ts: %lu",
                rc,
                mSystem.millis());
        }
        return rc;
    ...
    /**
     * Processes incoming messages and keep-alive transmission during given
     * interval. Call is interrupted if connection is lost. Mast be called
     * regularly.
     *
    ...
    unsigned long getIdleInterval()
    {
        ...
        {
            res = min(mSession.lastSentTimer.leftMs(),
                      mSession.lastRecvTimer.leftMs());
        }
        return res;
    }
...
private:
    ...
    public:
        ...
        Timer(const Timer &timer,
            ...
            : Timer(timer.mSystem)
        {
            unsigned long durationMs = (timer.leftMs() > maxDurationMs)
                                           ? maxDurationMs
                                           : timer.leftMs();
    ...
    struct Session
    {
        ...
        Session(const System &system)
            : lastSentTimer(system), lastRecvTimer(system),
    ...
    Error::type recvPacket(ReadPacketResult &result)
    {
        ...
        {
            return mNetwork.connected() ? Error::WAIT_TIMEOUT
        ...
        timer.reset();
        if (remLen > 0 &&
            (recvBytes(mRecvBuffer.get() + len, remLen, timer) != remLen))
        {
            return Error::NETWORK_FAILURE;
        }
        // Get packet type
    ...
    {
        MQTT_LOG_PRINTFLN("Wait for message, type: %i, tm: %lu ms",
                          packetType,
                          timer.leftMs());
        ReadPacketResult result;
        Error::type rc = Error::SUCCESS;
    ...
    Error::type cycle(ReadPacketResult &result)
    {
        ...
        {
            MQTT_LOG_PRINTFLN("Keepalive ack failure, ts: %lu",
                              mSystem.millis());
            mSession.reset();
        }
        if (isConnected())
    ...
    Error::type processPacket(enum msgTypes packetType)
    {
        ...
        case SUBACK:
            break;
        case PINGRESP:
            MQTT_LOG_PRINTFLN("Keepalive ack received, ts: %lu",
                              mSystem.millis());
        ...
        case PUBREL:
        {
            ...
            {
                int len = MQTTSerialize_ack(
                    mSendBuffer.get(), mSendBuffer.size(), PUBCOMP, 0, ackId);
                if (len > 0)
                {
                    rc = sendPacket(len);
                }
    ...
    Error::type keepalive()
    {
        ...
        if (!mSession.keepaliveSent
            ...
            (mSession.lastSentTimer.expired() ||
             (
                 // Do not send if another message is just sent => try to wait
                 // the response
                 ...
                 && mSession.lastRecvTimer.expired())))
        {
            ...
            int len =
                MQTTSerialize_pingreq(mSendBuffer.get(), mSendBuffer.size());
            if (len <= 0)
            {
                return Error::ENCODING_FAILURE;
            }
            ...
            {
                MQTT_LOG_PRINTFLN("Keepalive failure, ts: %lu",
                                  mSystem.millis());
                mSession.reset();
            }
            return rc;
    ...
    void deliverMessage(MQTTString &topic, Message &message)
    {
        ...
        for (int i = 0; i < mMessageHandlers.size(); ++i)
        {
            ...
            {
                if (MQTTPacket_equals(
                        &topic, (char *)(mMessageHandlers.get()[i].topic)) ||
                    isTopicMatched((char *)(mMessageHandlers.get()[i].topic),
                                   topic))
                {
                    MQTT_LOG_PRINTFLN("Deliver message for: %s",
                                      mMessageHandlers.get()[i].topic);
